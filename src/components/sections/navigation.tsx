        ...
        <div className="flex h-20 items-center justify-between">
          <Link href="/" className="flex items-center group" aria-label="VPAG - Versatile Performing Art Group">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-br from-yellow-500/20 to-yellow-600/20 rounded-xl blur-sm group-hover:blur-none transition-all duration-300"></div>
              <div className="relative bg-gradient-to-br from-zinc-900/50 to-zinc-800/50 rounded-xl p-2 border border-zinc-700/50 group-hover:border-yellow-500/30 transition-all duration-300">
                <Image
                  src="/vpag-logo.png"
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href 
                    ? 'text-yellow-400 bg-zinc-800/30' 
                    : 'text-zinc-300 hover:text-white'
                )}
              >
                {item.label}
                {pathname === item.href && (
                  <div className="absolute bottom-0 left-1/2 h-0.5 w-4 -translate-x-1/2 rounded-full bg-yellow-400"></div>
                )}
              </Link>
            ))}
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href
                    ? 'bg-zinc-800/50 text-yellow-400 border border-zinc-700/50'
